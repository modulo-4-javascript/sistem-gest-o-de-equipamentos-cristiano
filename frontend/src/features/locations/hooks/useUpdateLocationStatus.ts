...
// Hook de situação: expõe uma função updateStatus para alterar só a situação da localização.
export function useUpdateLocationStatus(): UpdateLocationStatusState {
  const [isLoading, setIsLoading] = useState(false)
  const [errorMessage, setErrorMessage] = useState('')

  // Recebe o ID e a nova situação, chama o PATCH do service e controla loading/erro.
